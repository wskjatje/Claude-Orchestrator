...
export type DesktopApi = {
  ...
  getWorkspaceHistory: () => Promise<{
    version: number;
    entries: { path: string; openedAt: number }[];
  }>;
  removeWorkspaceHistoryEntry: (
    path: string,
  ) => Promise<{ version: number; entries: { path: string; openedAt: number }[] }>;
  clearWorkspaceHistory: () => Promise<{
    version: number;
    entries: { path: string; openedAt: number }[];
  }>;
  ...
  workspaceBulkIngestFromHistory?: (payload: { texts: string[]; agentName?: string }) => Promise<{
    ok: boolean;
    error?: string;
    written?: string[];
    errors?: string[];
  ...
  fileStat?: (relPath: string) => Promise<{
    ok: boolean;
    exists: boolean;
    size?: number;
    error?: string;
  }>;
  /** 后台执行 graphify update 更新知识图谱，含 HTML 导出 */
  buildGraph?: () => Promise<{
    ok: boolean;
    output?: string;
    error?: string;
  ...
  nodeStatus?: () => Promise<{
    ok: boolean;
    counts?: { completed: number; inProgress: number; pending: number; optimizable: number; total: number };
    error?: string;
  }>;
  /** 图谱驱动并行执行：自动分解社区 → 并行分配 Agent → 汇总 */
  graphDecomposeAndExecute?: (payload: {
    task: string;
    modelId?: string;
    ...
    metadata?: {
      ...
      totalNodes: number;
      communityAgentMap: Record<string, string[]>;
      levelCount: number;
    };
    outputs?: Array<{
      agentName: string;
  ...
  workbenchGitCommitBranch: (payload: { reason: string }) => Promise<{
    ok: boolean;
    committed: boolean;
    branch?: string;
    commitHash?: string;
  ...
  orchestrationLoadChain: () => Promise<{
    ok: boolean;
    state?: OrchestrationState;
    error?: string;
  }>;
  orchestrationAdvanceChain: () => Promise<{
    ok: boolean;
    state?: OrchestrationState;
    error?: string;
  }>;
  ...
  orchestrationActivateChain?: (
    id: string,
  ) => Promise<{ ok: boolean; state?: OrchestrationState | null; error?: string | null }>;
  ...
  multiAgentExecuteParallel?: (payload: {
    agents: string[];
    task: string;
    modelId?: string;
    needSynthesis?: boolean;
  ...
  onAgentExecChanged: (
    fn: (data: { date?: string; stem?: string; status?: string }) => void,
  ) => () => void;
  scheduledTasksGet: () => Promise<{ ok: boolean; tasks?: ScheduledTask[]; error?: string }>;
  scheduledTasksSave: (
    tasks: ScheduledTask[],
  ) => Promise<{ ok: boolean; tasks?: ScheduledTask[]; error?: string }>;
  scheduledTasksRunNow: (taskId: string) => Promise<{ ok: boolean; error?: string }>;
  logsReadTail: (opts: {
    maxLines?: number;
    maxBytes?: number;
    source?:
      "app" | "claudeEvents" | "claudeDebugLatest" | "claudeHistoryJsonl" | "claudeLibraryLogs";
  ...
  agentDailyReportsListAgents: (
    date: string,
  ) => Promise<{ ok: boolean; date?: string; stems?: string[]; error?: string }>;
  ...
  dailyReportsGet: (
    date: string,
  ) => Promise<{ ok: boolean; date?: string; content?: string; missing?: boolean; error?: string }>;
  dailyReportsSave: (payload: {
    date: string;
    content: string;
  ...
  createClaudeAgentMarkdown: (stem?: string) => Promise<{
    ok: boolean;
    stem?: string;
    basename?: string;
    fullPath?: string;
  ...
  openClaudeAgentMarkdownFile: (basename: string) => Promise<{ ok: boolean; error?: string }>;
  ...
  readClaudeConfigJson: (name: "mcp.json" | "settings.json") => Promise<{
    ok: boolean;
    data?: unknown;
    raw?: string | null;
    missing?: boolean;
  ...
  bundledMcpCommandLines?: () => Promise<{
    ok: boolean;
