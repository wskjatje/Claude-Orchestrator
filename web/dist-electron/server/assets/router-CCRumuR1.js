...
const EMPTY_CHAT_SEARCH = {
  session: void 0,
  new: void 0,
  claudeResume: void 0
};
const appCss = "/assets/styles-DdTWFU0d.css";
let desktopHydrated = false;
function markDesktopHydrated() {
...
let prefsCache = { ...UI_PREFS_DEFAULTS, layoutStorage: {} };
let prefsCacheReady = false;
let saveTimer = null;
function normalizeLayoutStorage(raw) {
...
function normalizeUiPrefs(raw) {
  const themeMode = raw?.themeMode === "light" || raw?.themeMode === "dark" || raw?.themeMode === "system" ? raw.themeMode : UI_PREFS_DEFAULTS.themeMode;
  const defaultTerminalShell = raw?.defaultTerminalShell === "bash" || raw?.defaultTerminalShell === "zsh" ? raw.defaultTerminalShell : UI_PREFS_DEFAULTS.defaultTerminalShell;
  return {
    themeMode,
    bridgeUrl: typeof raw?.bridgeUrl === "string" ? raw.bridgeUrl : UI_PREFS_DEFAULTS.bridgeUrl,
    localSecret: typeof raw?.localSecret === "string" ? raw.localSecret : UI_PREFS_DEFAULTS.localSecret,
    defaultSessionTag: typeof raw?.defaultSessionTag === "string" && raw.defaultSessionTag.trim() ? raw.defaultSessionTag.trim() : UI_PREFS_DEFAULTS.defaultSessionTag,
    layoutStorage: normalizeLayoutStorage(raw?.layoutStorage),
    skipCheckpointConfirm: raw?.skipCheckpointConfirm === true,
    defaultTerminalShell
  };
}
function applyPrefsCache(prefs) {
  prefsCache = normalizeUiPrefs(prefs);
  prefsCacheReady = true;
}
function getUiPrefsCache() {
  return prefsCache;
}
function getUiPrefsCacheReady() {
  return prefsCacheReady;
}
function patchUiPrefsCache(partial) {
  prefsCache = normalizeUiPrefs({
    ...prefsCache,
    ...partial,
    layoutStorage: partial.layoutStorage ? { ...prefsCache.layoutStorage, ...partial.layoutStorage } : prefsCache.layoutStorage
  });
}
function scheduleSaveUiPrefs(partial, debounceMs = 300) {
...
function collectLegacyBrowserPrefs(current) {
  const patch = {};
  ...
  if ((legacyShell === "bash" || legacyShell === "zsh") && current.defaultTerminalShell === UI_PREFS_DEFAULTS.defaultTerminalShell) {
...
async function migrateBrowserPrefsToProjectDb() {
...
async function loadUiPrefsFromProjectDb() {
  const api = getDesktop();
  if (!api?.getUiPrefs) {
  ...
  const r2 = await api.getUiPrefs();
  const prefs = r2.ok && r2.prefs ? normalizeUiPrefs(r2.prefs) : { ...UI_PREFS_DEFAULTS, layoutStorage: {} };
  applyPrefsCache(prefs);
  return prefs;
}
async function saveUiPrefsToProjectDb(partial) {
  const api = getDesktop();
  ...
  const merged = normalizeUiPrefs({
    ...cur,
    ...partial,
    layoutStorage: partial.layoutStorage ? { ...cur.layoutStorage, ...partial.layoutStorage } : cur.layoutStorage
  });
  const r2 = await api.saveUiPrefs(merged);
  if (r2.ok) {
    applyPrefsCache(r2.prefs ? normalizeUiPrefs(r2.prefs) : merged);
...
function ThemeProvider({ children }) {
  ...
  const [prefsLoaded, setPrefsLoaded] = reactExports.useState(false);
  reactExports.useEffect(() => {
    let cancelled = false;
    void (async () => {
      ...
      const r2 = initial === "system" ? getSystem() : initial;
      setResolved(r2);
      apply(r2);
      setPrefsLoaded(true);
    })();
  ...
  const setMode = (m) => {
    setModeState(m);
    const r2 = m === "system" ? getSystem() : m;
    setResolved(r2);
    apply(r2);
    void saveUiPrefsToProjectDb({ themeMode: m });
  };
  return /* @__PURE__ */ jsxRuntimeExports.jsx(ThemeContext.Provider, { value: { mode, resolved, setMode, prefsLoaded }, children });
}
function useTheme() {
...
let outputSnapshot = buildOutputSnapshot();
...
function subscribeOutputLog(fn) {
  listeners$1.add(fn);
...
function appendOutput(id, line) {
  const c = channel(id);
  const stamp = (/* @__PURE__ */ new Date()).toLocaleTimeString(void 0, { hour12: false });
  c.lines.push(`[${stamp}] ${line}`);
  if (c.lines.length > MAX_LINES) c.lines.splice(0, c.lines.length - MAX_LINES);
  emit();
}
function appendOutputRaw(id, text) {
  const c = channel(id);
...
const APP_NAME = "Claude Orchestrator";
const BRIDGE_OFFLINE_BANNER = "无法连接本机服务。请确认 Claude Orchestrator 已启动，然后刷新页面。";
const BRIDGE_OFFLINE_LEGACY = "本机服务未连接。请完成连接配置后刷新页面。";
const BRIDGE_OFFLINE_TOAST = "未连接本机服务，请启动应用后重试";
const BRIDGE_CONNECTED_VERSION = "已连接";
const BRIDGE_OFFLINE_VERSION = "未连接";
const BRIDGE_ELECTRON_VERSION = "桌面版";
const MSG_BRIDGE_OFFLINE = BRIDGE_OFFLINE_TOAST;
const MSG_API_NOT_READY = "功能暂不可用，请重启 Claude Orchestrator 后刷新页面。";
const MSG_API_OUTDATED = "应用版本过旧，请重启 Claude Orchestrator 后再试。";
const LOCAL_ONLY_HINT = "需连接本机服务后可用";
const DEMO_LIST_BANNER = "示例数据。连接本机后点「刷新」加载。";
const AGENTS_DISK_SYNC_HINT = "已与 Claude Code Agent 目录同步。";
const AGENTS_DEMO_HINT = "示例列表。连接本机后点「刷新」加载。";
const PAGE_DESC = {
  overview: "连接、用量与运行概况",
  agents: "管理本机 Agent",
  skills: "管理本机 Skill",
...
const GIT_PUSH_HINT = "推送时写入前端实现说明（各页面 UI 与功能）。";
const GIT_PUSH_HINT_DETAIL = "自动生成 docs/claude-orchestrator-apps.md 并写入 Git 提交正文；Agent/Skill/链/MCP 一并导出。";
const GIT_PUSH_REASON_LABEL = "本次变更（可选）";
const GIT_PUSH_REASON_PLACEHOLDER = "留空则仅推送「前端实现说明」；第一行作提交标题，其余行为变更要点";
const CHAINS_INFO_HINT = "官方链已内置；对话中用 /chain 调用。";
const MCP_INFO_HINT = "配置保存在本机；详情内可健康检查。";
const MCP_EMPTY_BANNER = "尚未配置 MCP。点「添加」选择模板。";
const REPORTS_INFO_HINT = "Agent 与任务链执行会自动写入日报。";
const OVERVIEW_INFO_HINT = "工作区 → 侧栏；模型 → 设置；MCP → MCP 页。";
const AGENTS_EDITOR_HINT = "右侧编辑；「工作流优化」可生成修订稿。";
const SKILLS_EDITOR_HINT = "本机目录扫描；编辑对应文件即可。";
const SCHEDULED_BANNER_ONLINE = "每 30 秒检查；开关即时保存。";
const MODELS_PANEL_FOOTER = "与聊天页模型下拉同步；使用中的不可删。";
const MODELS_EMPTY_HINT = "尚未添加云模型。点「添加云模型」开始配置。";
const USAGE_SECTION_DESC = "云端与本地用量汇总。";
const TODAY_KPIS_DESC = "今日关键指标。";
const CONFIRM_WRITE_SECTION_HINT = "回复无落盘块时写入此路径。";
const CONFIRM_WRITE_FOOTER = "部分角色需在聊天中确认后再写入。";
const HELP_SECTION_DESC = {
  guide: "Agent、Skill、任务链与 MCP",
  external: "官方文档与社区",
  shortcuts: "macOS（Windows 用 Ctrl）",
  about: "版本与反馈"
};
const SETTINGS_SAVED_PROJECT = "设置保存在当前项目中，不会同步到云端。";
const WORKSPACE_HISTORY_HINT = "记录最近打开的项目文件夹。";
const WORKSPACE_API_MISSING = "未连接本机服务。请启动 Claude Orchestrator 并刷新本页。";
const CHOOSE_WORKSPACE_OFFLINE_TITLE = "无法连接本机服务";
const CHOOSE_WORKSPACE_OFFLINE_DESC = "请启动 Claude Orchestrator 并刷新页面。";
const COMPOSER_PLACEHOLDER = "输入内容，@ 引用文件，/ 使用命令";
const BROWSER_MODE_CHAT_MESSAGE = "请使用 Claude Orchestrator 桌面窗口打开，以使用完整聊天、工作区与对话记录。";
const WORKSPACE_TREE_OFFLINE = "未连接本机时无法加载文件树。";
const TERMINAL_OFFLINE = "未连接本机时无法打开终端。请启动 Claude Orchestrator 后重试。";
const WORKBENCH_SIDEPANEL_OFFLINE = "请连接本机服务并选择工作目录。";
const OPENCLAW_TOKEN_UNAVAILABLE = "当前模式下无法读取 OpenClaw 令牌。";
const OPENCLAW_TOKEN_PLACEHOLDER = "可选填写";
const PROJECT_PREVIEW_UNSUPPORTED = "项目预览功能暂不可用，请重启 Claude Orchestrator 后重试。";
const PROJECT_PREVIEW_API_MISSING = "预览接口未就绪，请重启 Claude Orchestrator 后再试。";
const PROJECT_PREVIEW_STOP_UNSUPPORTED = "当前无法停止预览，请在本机终端结束相关进程。";
const REPORTS_API_OFFLINE = "日报功能需要本机服务，请重启应用后重试。";
const SCHEDULED_OFFLINE = "未连接本机：定时任务不会在本地自动执行。";
const WORKSPACE_HISTORY_API_OFFLINE = "无法读取打开记录，请重启 Claude Orchestrator 后刷新本页。";
const WORKSPACE_BROWSE_HINT = "点击「浏览目录」可在本机选择项目文件夹。";
const RPC_CONNECTION_ERROR = (detail) => `无法连接本机服务：${detail}。请确认 Claude Orchestrator 已启动。`;
const RPC_CONNECTION_TIMEOUT_HINT = "若长时间无响应，可尝试缩短对话上下文或重启应用。";
const RPC_BASE = "/api";
const QUIET_RPC = /* @__PURE__ */ new Set([
  "workspace:get",
...
async function rpc(channel2, ...args) {
  ...
  try {
    res = await fetch(`${RPC_BASE}/rpc`, {
      ...
      body: JSON.stringify({ channel: channel2, args })
    });
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    appendOutput("bridge", `RPC ${channel2} 不可达: ${msg}`);
    const hint = /fetch failed|ECONNREFUSED|ETIMEDOUT|socket hang up|network/i.test(msg) ? `（${RPC_CONNECTION_TIMEOUT_HINT}）` : "";
  ...
  if (!res.ok) {
    const errObj = data && typeof data === "object" && "error" in data ? String(data.error) : text;
    appendOutput("bridge", `RPC ${channel2} 失败 (${res.status}): ${errObj || text}`);
    throw new Error(errObj || `RPC ${channel2} failed (${res.status})`);
  }
  if (!QUIET_RPC.has(channel2)) {
    appendOutput("bridge", `RPC ${channel2} 完成`);
  }
  return data;
}
async function pingWebBridgeHealth() {
  try {
...
function onEvent(channel2, fn) {
  if (!eventHandlers.has(channel2)) eventHandlers.set(channel2, /* @__PURE__ */ new Set());
  eventHandlers.get(channel2).add(fn);
  return () => eventHandlers.get(channel2)?.delete(fn);
}
function onBridgeEvent(channel2, fn) {
  return onEvent(channel2, fn);
}
function connectBridgeEvents() {
  ...
  try {
    ...
    ws.onclose = () => {
      window.setTimeout(connectBridgeEvents, 3e3);
    };
  } catch {
    window.setTimeout(connectBridgeEvents, 3e3);
  }
...
function installDesktopBridge() {
  ...
  const desktop = {
    ...
    chooseWorkspace: (manualPath) => rpc("workspace:choose", typeof manualPath === "string" ? manualPath : void 0),
    getWorkspaceHistory: () => rpc("workspace:history:get"),
    removeWorkspaceHistoryEntry: (path) => rpc("workspace:history:remove", path),
    ...
    readReferenceFilesAsImageAttachments: (filePaths) => rpc("reference-files:readAsImageAttachments", filePaths),
    ...
    workbenchGitCheckUpstream: (payload) => rpc("workbench-git:checkUpstream", payload ? [payload] : []),
    workbenchGitPullUpstream: (payload) => rpc("workbench-git:pullUpstream", payload ? [payload] : []),
    workbenchGitPullPersonal: (payload) => rpc("workbench-git:pullPersonal", payload ? [payload] : []),
    ...
    getOpenclawGatewayToken: () => Promise.resolve({ ok: false, error: "未配置", token: void 0 }),
...
const DesktopReadyContext = reactExports.createContext(false);
function DesktopHydrationProvider({ children }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(DesktopReadyContext.Provider, { value: ready, children });
}
function useDesktopReady() {
  return reactExports.useContext(DesktopReadyContext);
}
function useHasDesktop() {
...
function relatedArtifactPathsForAgent(agentName) {
  ...
  return [.../* @__PURE__ */ new Set([...upstream, own])].filter(Boolean);
}
function buildAgentArtifactPathHint(stemRaw) {
  ...
  return `【落盘路径·强制】你是 global://${stem}。写入工作区时 \`\`\`workspace-write\`\`\` 的 path **默认必须是** \`${path}\`；同一 Agent 多次交付仍用此路径（追加或覆盖须在正文说明）；禁止写入其它 Agent 专属文件（如 ${examples}），除非用户明确要求合并到指定路径。未在映射表中的角色默认 \`docs/agents/{stem}.md\` 或 \`docs/{类别}/{角色名}.md\`。`;
}
const MANUAL_CONFIRM_WRITE_STEMS = /* @__PURE__ */ new Set([
  "product-manager",
  "project-manager",
...
const CHAIN_STEP_BOOTSTRAP_READ_PATHS = ["CLAUDE.md", "README.md", "docs/README.md"];
const IMPLEMENTATION_STEMS = /* @__PURE__ */ new Set([
  "frontend-engineer",
  "backend-engineer",
  "devops-engineer"
]);
function buildChainStepReadBlock(stemRaw) {
  const stem = normalizeAgentStem(stemRaw);
  if (!stem) return "";
...
function buildChainStepWriteBlock(stemRaw) {
  const stem = normalizeAgentStem(stemRaw);
  if (!stem) return "";
  const path = defaultArtifactPathForAgent(stem);
  ...
  if (manual) {
    lines.push(
      "【落盘·任务链】跑链/Multi-Agent 时须输出合法 ```workspace-write``` JSON（path 默认 `" + path + "`）；系统据此真正写盘。禁止仅写「已落盘」口述或虚构「【工作区已写入】」。",
      "【落盘·单聊】普通对话（非跑链）仍须先询问用户是否落盘，获明确同意后再写 workspace-write。"
    );
  } else {
    lines.push(
      "【落盘·自动】须输出合法 ```workspace-write``` JSON 写入默认路径；若无合法围栏，系统会将摘要写入 docs/chain-steps/{任务ID}-" + stem + ".md 或自动写入 `" + path + "`。",
...
const UI_IMPLEMENTATION_STEMS = /* @__PURE__ */ new Set([
  "design-ui-designer",
  "design-ux-architect",
  "ui-ux-designer",
...
const UI_TASK_RE = /(?:登录页|登录页面|注册|忘记密码|页面|界面|UI|HTML|html|css|mockup|原型|扫码|第三方登录|视觉|布局)/i;
...
function buildRoutingToolHint(mode, preset) {
  if (mode === "local-mcp") {
    return [.../* @__PURE__ */ new Set([...preset.tools, "ollama_list_models", "ollama_chat", "workspace-write"])].join("、");
  }
  return [...preset.tools, "MCP（按需）", "workspace-write"].join("、");
}
function buildAgentScopeGuard(stem, body) {
  ...
  return "【本职边界·强制】用户任务含页面/UI，但 global://" + stem + " 不包含画界面；须按 ~/.claude/agents 中该角色定义作答，并说明应委派给哪个 Agent。";
}
function buildAgentRoutedInstruction(stem, body, mode) {
  const cleanBody = body.trim();
  const lock = `【角色锁定】你只扮演 global://${stem}；禁止自称其它 Agent，禁止混淆产品经理与项目经理等不同职务。禁止在同一回复中分段扮演「Agent: 需求分析员/架构师/前端…」；其它职务须由任务链下一步或用户另发 /agent 指令调用。`;
  const scopeGuard = buildAgentScopeGuard(stem, cleanBody);
  const executeHint = !cleanBody ? UI_IMPLEMENTATION_STEMS.has(stem) ? "【执行指令】你已接管本轮。请阅读上文用户目标与 assistant 已有设计说明，直接产出**新的**可交付物（完整 HTML/CSS 代码，或 ```workspace-write``` 写入工作区相对路径）。禁止逐字重复上一条 assistant 回复。" : `【执行指令】你已接管 global://${stem}。请仅按该 Agent 本职阅读上文并产出对应交付物；禁止写其它 Agent 负责的内容（尤其禁止 UI 设计稿若你不是设计/前端 Agent）。` : "";
  const preset = AGENT_ROUTING_PRESETS[stem];
  const designDeliver = UI_IMPLEMENTATION_STEMS.has(stem) ? "【交付】须给出可直接使用的页面代码（HTML+CSS）或 workspace-write 落盘；不要只写抽象设计要点清单。" : "";
  const artifactPathHint = buildAgentArtifactPathHint(stem);
  if (!preset) {
    return [lock, scopeGuard, artifactPathHint, executeHint, designDeliver, cleanBody].filter(Boolean).join("\n\n").trim();
  }
  const routingBlock = [
...
function getAgentSkillToolBundle(agentStem) {
  const stem = agentStem.trim();
  if (!stem) return null;
  const preset = AGENT_ROUTING_PRESETS[stem];
  if (!preset) return null;
  const skillStems = /* @__PURE__ */ new Set();
  if (preset) {
...
function resolveAgentSkillBundleFromMeta(agentStem, parsed, agentEntry = {}) {
  const stem = agentStem.trim();
  const skillStems = /* @__PURE__ */ new Set();
  ...
  const tools = parsed.tools?.length ? parsed.tools : agentEntry.tools?.length ? agentEntry.tools : ["read", "edit"];
  return {
    agentStem: stem,
    skillStems: [...skillStems],
    tools
...
function getSkillDefinition(stem, agentStem) {
  const s = stem.trim();
...
function dedupeSkillStems(stems) {
  const seen = /* @__PURE__ */ new Set();
  const out = [];
...
function skillFileStemsForAgent(agentStem, agentSkills) {
...
function mergeAgentStepSkills(agentStem, stepSkills = [], agentSkills, availableStems) {
...
function allSkillFilesForAgentStems(stems) {
...
function suggestedSkillStemsForAgent(agentStem, availableStems, agentSkills) {
  return mergeAgentStepSkills(agentStem, [], agentSkills, availableStems);
}
function resolveChainStepSkills(agentStem, agents = [], availableStems) {
  const stem = agentStem.trim();
  if (!stem) return [];
  const agentRow = agents.find((a) => a.stem === stem);
  return suggestedSkillStemsForAgent(stem, /* @__PURE__ */ new Set(), agentRow?.skills);
}
function skillsUnionFromSteps(steps, agents = []) {
  const set = /* @__PURE__ */ new Set();
  for (const step2 of steps) {
    const stem = String(step2.agentName ?? "").trim();
    if (!stem) continue;
...
function singleTemplate(id, name, description, stem, taskId, body, vars, skillsOverride) {
...
const GENERIC_CTX = "【上下文】以当前工作区与用户本次对话中的需求为准；若工作区已有相关文档，优先阅读后再执行。";
const GENERIC_DELIVERY = "【交付】按对话中已确认的范围、验收标准与工作区相关文档执行。";
const SANSHENGLIUBU_GOVERNANCE = "【三省六部·执行顺序】须遵守 MCP → Skills → Agent；底层工具（local-time 等）可随时调用。";
...
${body}`, skillsOverride);
  return { ...base, mcps: ["sanshengliubu"] };
}
function governancePipelineTemplate(id, name, description, agents, steps, vars = []) {
  return {
    id,
    category: "pipeline",
    name,
...
const SELF_LEARNING_AGENT_STEMS = [
  "product-manager",
  "product-sprint-prioritizer",
  "project-manager",
  "software-architect",
...
function agentSelfReviewBody(stem) {
...
【任务】若有基础设施/发布变更：步骤、回滚与验证清单`
  ...
  governancePipelineTemplate(
    ...
    [
      ...SELF_LEARNING_AGENT_STEMS.map(
        (stem, i) => sanshengliubuStep(stem, `LEARN-${String(i + 1).padStart(2, "0")}`, agentSelfReviewBody(stem))
      ),
      sanshengliubuStep(
        "project-manager",
        "LEARN-汇总",
...
function substituteChainTemplateVars(text, vars) {
...
function applyChainTemplate(template, vars) {
...
const CHAIN_TEMPLATE_CATEGORY_LABEL = {
  single: "单 Agent + Skill",
  pipeline: "多 Agent 流水线"
};
async function syncOfficialGenericChains() {
  const api = getDesktop();
  ...
  const r2 = await api.orchestrationEnsureOfficialChains({ items });
  return { ok: Boolean(r2.ok), synced: r2.synced };
}
function buildChainWorkflowBadge(opts) {
...
function setChainRunningFromServer(running) {
  if (chainRunning === running) return;
  chainRunning = running;
  notify();
}
function requestStopChainExecution() {
  const api = getDesktop();
...
async function runOrchestrationChainInBackground(opts) {
  const api = getDesktop();
  if (!api) {
  ...
  if (!opts?.skipConfirm && !confirm(
    mode === "claude-code" ? "将从任务链状态依次调用 Agent 执行（可能较久）。是否继续？" : "将以本地 MCP 模式依次调用 Agent 执行（可能较久）。是否继续？"
  )) {
    return;
  }
  ...
  try {
    const started = await api.orchestrationStartChainRun({
      pinnedSessionId: opts?.pinnedSessionId?.trim() || void 0
    });
    if (!started.ok) {
...
const OrchestrationContext = reactExports.createContext(null);
...
function OrchestrationExecutionProvider({ children }) {
  const desktopReady = useDesktopReady();
  const location = useLocation();
  const [chainRunning2, setChainRunning] = reactExports.useState(false);
  const chainRunningRef = reactExports.useRef(false);
  const [sessionRevision, setSessionRevision] = reactExports.useState(0);
  const [chainStatusBadge, setChainStatusBadge] = reactExports.useState({
    label: "链：检查中",
    tone: "neutral"
  });
  const applyRunning = reactExports.useCallback((running) => {
    chainRunningRef.current = running;
    setChainRunning(running);
    setChainRunningFromServer(running);
  }, []);
  const refreshChainStatusBadge = reactExports.useCallback(async (opts) => {
    const api = getDesktop();
  ...
  const syncExecutionState = reactExports.useCallback(async () => {
    const api = getDesktop();
    if (!api?.orchestrationGetChainRunStatus) {
  ...
  reactExports.useEffect(() => {
    ...
    const intervalMs = chainRunning2 ? 800 : 2200;
  ...
  }, [desktopReady, chainRunning2, syncExecutionState]);
  reactExports.useEffect(() => {
    if (!desktopReady) return;
    const api = getDesktop();
  ...
  const runOrchestrationChain = reactExports.useCallback(
    async (opts) => {
  ...
  const stopChainExecution = reactExports.useCallback(() => {
...
function useOrchestrationExecution() {
  const ctx = reactExports.useContext(OrchestrationContext);
  return ctx ?? ORCHESTRATION_FALLBACK;
}
function useChatSessionRevision() {
...
function SchedulerNotifications() {
  const ready = useDesktopReady();
  reactExports.useEffect(() => {
    if (!ready) return;
    const api = getDesktop();
...
function NotFoundComponent() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex min-h-screen items-center justify-center bg-background px-4", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-md text-center", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "mt-2 text-sm text-muted-foreground", children: "你访问的页面不存在，或已被移动。" }),
...
function RootComponent() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(ThemeProvider, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(DesktopHydrationProvider, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(OrchestrationExecutionProvider, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(TooltipProvider, { delayDuration: 150, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(Outlet, {}),
    /* @__PURE__ */ jsxRuntimeExports.jsx(SchedulerNotifications, {}),
    /* @__PURE__ */ jsxRuntimeExports.jsx(Toaster2, { position: "bottom-center", duration: 2200, richColors: false, closeButton: true, visibleToasts: 1 })
  ] }) }) }) }) });
}
const $$splitComponentImporter$d = () => import("./workspaces--TIMtsG7.js");
const Route$d = createFileRoute("/workspaces")({
  head: () => ({
    meta: [{
      title: "工作目录 · Claude Orchestrator"
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$d, "component")
});
const USAGE_RANGE_PRESETS = ["今天", "7天", "30天"];
function todayStartMs(now = Date.now()) {
  const d = new Date(now);
  d.setHours(0, 0, 0, 0);
  return d.getTime();
}
function rangeToBounds(range, now = Date.now()) {
  const end = now;
  if (range === "今天") return { start: todayStartMs(now), end };
  if (range === "7天") return { start: end - 7 * 24 * 60 * 60 * 1e3, end };
  if (range === "30天") return { start: end - 30 * 24 * 60 * 60 * 1e3, end };
  return { start: 0, end };
}
...
function parseUsageRange(raw) {
  if (raw === "7天" || raw === "30天") return raw;
  return "今天";
}
const $$splitComponentImporter$c = () => import("./usage-BTU5dmpx.js");
const Route$c = createFileRoute("/usage")({
...
const $$splitComponentImporter$b = () => import("./skills-SksYFO7W.js");
const Route$b = createFileRoute("/skills")({
  head: () => ({
    meta: [{
      title: "Skill · Claude Orchestrator"
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$b, "component")
});
const $$splitComponentImporter$a = () => import("./settings-DDm8MtOD.js");
const Route$a = createFileRoute("/settings")({
  head: () => ({
    meta: [{
      title: "应用设置 · Claude Orchestrator"
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$a, "component")
});
const $$splitComponentImporter$9 = () => import("./scheduled-DRTFEB18.js");
const Route$9 = createFileRoute("/scheduled")({
  head: () => ({
    meta: [{
      title: "定时任务 · Claude Orchestrator"
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$9, "component")
});
const $$splitComponentImporter$8 = () => import("./reports-Ci7yvcSb.js");
const Route$8 = createFileRoute("/reports")({
  head: () => ({
    meta: [{
      title: "智能体执行日报 · Claude Orchestrator"
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$8, "component")
});
const $$splitComponentImporter$7 = () => import("./overview-BE7c7Yhj.js");
const Route$7 = createFileRoute("/overview")({
  validateSearch: (search) => ({
    range: parseUsageRange(search.range)
  }),
  head: () => ({
    meta: [{
      title: "概览 · Claude Orchestrator"
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$7, "component")
});
const $$splitComponentImporter$6 = () => import("./logs-BrUMYZfc.js");
const Route$6 = createFileRoute("/logs")({
  head: () => ({
    meta: [{
      title: "日志 · Claude Orchestrator"
    }]
  }),
  validateSearch: (search) => ({
    tab: search.tab === "claude" || search.tab === "trace" ? search.tab : "workbench",
    stem: typeof search.stem === "string" ? search.stem : ""
  }),
  component: lazyRouteComponent($$splitComponentImporter$6, "component")
});
const $$splitComponentImporter$5 = () => import("./help-jHAh1dr2.js");
const Route$5 = createFileRoute("/help")({
  head: () => ({
    meta: [{
      title: "帮助与链接 · Claude Orchestrator"
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$5, "component")
});
const $$splitComponentImporter$4 = () => import("./editor-C1rSJX6A.js");
const Route$4 = createFileRoute("/editor")({
  component: lazyRouteComponent($$splitComponentImporter$4, "component")
});
const $$splitComponentImporter$3 = () => import("./comms-CKgDpTtz.js");
const Route$3 = createFileRoute("/comms")({
  head: () => ({
    meta: [{
      title: "MCP 服务器 · Claude Orchestrator"
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$3, "component")
});
const $$splitComponentImporter$2 = () => import("./chains-CYw9vcCu.js");
const Route$2 = createFileRoute("/chains")({
  head: () => ({
    meta: [{
      title: "任务链 · Claude Orchestrator"
    }]
  }),
  validateSearch: (search) => ({
...
const $$splitComponentImporter$1 = () => import("./agents-C8zgTGBG.js");
const Route$1 = createFileRoute("/agents")({
  head: () => ({
    meta: [{
      title: "智能体 · Claude Orchestrator"
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$1, "component")
});
const $$splitComponentImporter = () => import("./index-Cl_CBDGP.js");
const Route2 = createFileRoute("/")({
  validateSearch: (search) => ({
    session: typeof search.session === "string" && search.session.trim() ? search.session.trim() : void 0,
    new: search.new === "1" || search.new === true ? true : void 0,
    claudeResume: typeof search.claudeResume === "string" && search.claudeResume.trim() ? search.claudeResume.trim() : void 0
  }),
  head: () => ({
    meta: [{
      title: "聊天 · Claude Orchestrator"
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter, "component")
});
const WorkspacesRoute = Route$d.update({
  id: "/workspaces",
  path: "/workspaces",
  getParentRoute: () => Route$e
});
const UsageRoute = Route$c.update({
  id: "/usage",
  path: "/usage",
  getParentRoute: () => Route$e
});
...
const ScheduledRoute = Route$9.update({
  id: "/scheduled",
  path: "/scheduled",
  getParentRoute: () => Route$e
});
...
const OverviewRoute = Route$7.update({
  id: "/overview",
  path: "/overview",
  getParentRoute: () => Route$e
});
const LogsRoute = Route$6.update({
  id: "/logs",
  path: "/logs",
  getParentRoute: () => Route$e
});
const HelpRoute = Route$5.update({
  id: "/help",
  path: "/help",
  getParentRoute: () => Route$e
});
const EditorRoute = Route$4.update({
  id: "/editor",
  path: "/editor",
  getParentRoute: () => Route$e
});
const CommsRoute = Route$3.update({
  id: "/comms",
  path: "/comms",
  getParentRoute: () => Route$e
});
const ChainsRoute = Route$2.update({
  id: "/chains",
  path: "/chains",
  getParentRoute: () => Route$e
});
const AgentsRoute = Route$1.update({
  id: "/agents",
  path: "/agents",
  getParentRoute: () => Route$e
});
...
export {
  ...
  pingWebBridgeHealth as A,
  BRIDGE_OFFLINE_TOAST as B,
  CHOOSE_WORKSPACE_OFFLINE_DESC as C,
  DEMO_LIST_BANNER as D,
  MSG_BRIDGE_OFFLINE as E,
  OPENCLAW_TOKEN_PLACEHOLDER as F,
  GIT_PUSH_REASON_LABEL as G,
  rangeToBounds as H,
  USAGE_RANGE_PRESETS as I,
  formatTokenCount as J,
  useNavigate as K,
  LOCAL_ONLY_HINT as L,
  MODELS_EMPTY_HINT as M,
  Route$7 as N,
  OPENCLAW_TOKEN_UNAVAILABLE as O,
  PAGE_DESC as P,
  OVERVIEW_INFO_HINT as Q,
  REPORTS_API_OFFLINE as R,
  SKILLS_EDITOR_HINT as S,
  TODAY_KPIS_DESC as T,
  USAGE_SECTION_DESC as U,
  Route$6 as V,
  WORKSPACE_HISTORY_API_OFFLINE as W,
  HELP_SECTION_DESC as X,
  useControllableState as Y,
  Primitive as Z,
  useId as _,
  CHOOSE_WORKSPACE_OFFLINE_TITLE as a,
  buildAgentRoutedInstruction as a$,
  Presence as a0,
  createContextScope as a1,
  useComposedRefs as a2,
  useCallbackRef as a3,
  useLayoutEffect2 as a4,
  MSG_API_NOT_READY as a5,
  MCP_INFO_HINT as a6,
  MCP_EMPTY_BANNER as a7,
  MSG_API_OUTDATED as a8,
  Route$2 as a9,
  PROJECT_PREVIEW_API_MISSING as aA,
  PROJECT_PREVIEW_STOP_UNSUPPORTED as aB,
  WORKBENCH_SIDEPANEL_OFFLINE as aC,
  getUiPrefsCache as aD,
  patchUiPrefsCache as aE,
  useTheme as aF,
  TERMINAL_OFFLINE as aG,
  appendOutputRaw as aH,
  subscribeOutputLog as aI,
  getOutputSnapshot as aJ,
  clearOutput as aK,
  OUTPUT_CHANNELS as aL,
  scheduleSaveUiPrefs as aM,
  appendOutput as aN,
  relatedArtifactPathsForAgent as aO,
  onBridgeEvent as aP,
  useSize as aQ,
  composeRefs as aR,
  Portal$1 as aS,
  createContext2 as aT,
  DismissableLayer as aU,
  Route2 as aV,
  useChatSessionRevision as aW,
  BROWSER_MODE_CHAT_MESSAGE as aX,
  EMPTY_CHAT_SEARCH as aY,
  COMPOSER_PLACEHOLDER as aZ,
  reactDomExports as a_,
  useOrchestrationExecution as aa,
  CHAIN_TEMPLATE_DEFAULTS as ab,
  syncOfficialGenericChains as ac,
  CHAIN_TEMPLATES as ad,
  CHAINS_INFO_HINT as ae,
  CHAIN_TEMPLATE_CATEGORY_LABEL as af,
  resolveChainStepSkills as ag,
  applyChainTemplate as ah,
  skillsUnionFromSteps as ai,
  CHAIN_TEMPLATE_VAR_LABELS as aj,
  suggestedSkillStemsForAgent as ak,
  dedupeSkillStems as al,
  resolveAgentSkillBundleFromMeta as am,
  getSkillDefinition as an,
  AGENTS_EDITOR_HINT as ao,
  AGENTS_DISK_SYNC_HINT as ap,
  AGENTS_DEMO_HINT as aq,
  Tooltip as ar,
  TooltipTrigger as as,
  TooltipContent as at,
  defaultArtifactPathForAgent as au,
  normalizeAgentStem as av,
  agentRequiresManualConfirmWrite as aw,
  agentAutoWritesToProject as ax,
  WORKSPACE_TREE_OFFLINE as ay,
  PROJECT_PREVIEW_UNSUPPORTED as az,
  useHasDesktop as b,
  clsx as b0,
  Root2 as b1,
  Anchor as b2,
  createPopperScope as b3,
  dispatchDiscreteCustomEvent as b4,
  Content as b5,
  Arrow as b6,
  APP_NAME as b7,
  BRIDGE_ELECTRON_VERSION as b8,
  BRIDGE_CONNECTED_VERSION as b9,
  BRIDGE_OFFLINE_VERSION as ba,
  useLocation as bb,
  BRIDGE_OFFLINE_LEGACY as bc,
  router as bd,
  WORKSPACE_API_MISSING as c,
  WORKSPACE_BROWSE_HINT as d,
  cn as e,
  WORKSPACE_HISTORY_HINT as f,
  getDesktop as g,
  MODELS_PANEL_FOOTER as h,
  isWebBridge as i,
  CONFIRM_WRITE_SECTION_HINT as j,
  CONFIRM_WRITE_FOOTER as k,
  SETTINGS_SAVED_PROJECT as l,
  SETTINGS_TAB_HINT as m,
  GIT_PUSH_REASON_PLACEHOLDER as n,
  GIT_PUSH_HINT as o,
  GIT_PUSH_HINT_DETAIL as p,
  hasDesktop as q,
  SCHEDULED_OFFLINE as r,
  SCHEDULED_BANNER_ONLINE as s,
  toast as t,
  useDesktopReady as u,
  REPORTS_INFO_HINT as v,
  BRIDGE_OFFLINE_BANNER as w,
  Link as x,
  loadUiPrefsFromProjectDb as y,
  saveUiPrefsToProjectDb as z
};
