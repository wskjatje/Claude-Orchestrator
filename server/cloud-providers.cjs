...
function normalizeProviderEnv(env) {
  ...
  if (next.ANTHROPIC_BASE_URL) {
    next.ANTHROPIC_BASE_URL = normalizeAnthropicBaseUrl(
...
const KNOWN_CLOUD_PROVIDERS = [
  {
    name: "Anthropic",
    needsCcr: false,
    defaultPricing: { inputPer1M: 3, outputPer1M: 15 },
  },
  {
    name: "DeepSeek",
    needsCcr: false,
    directApi: true,
    directApiBase: "https://api.deepseek.com/v1",
    fetchModelsStrategy: "openai-compatible",
    fetchModelsPath: "/v1/models",
    defaultPricing: { inputPer1M: 0.27, outputPer1M: 1.1 },
  },
  {
    name: "OpenAI",
    needsCcr: false,
    ...
    fetchModelsStrategy: "openai-compatible",
    defaultPricing: { inputPer1M: 2.5, outputPer1M: 10 },
  },
  {
    name: "Google Gemini",
    needsCcr: false,
    directApi: true,
    directApiBase: "https://generativelanguage.googleapis.com/v1beta/models/",
    fetchModelsStrategy: "gemini",
  ...
  },
  {
    ...
    needsCcr: false,
    directApi: true,
    directApiBase: "https://api.groq.com/openai/v1",
    fetchModelsStrategy: "openai-compatible",
    defaultPricing: { inputPer1M: 0.2, outputPer1M: 0.6 },
  },
  {
    name: "Mistral",
    needsCcr: false,
    directApi: true,
    directApiBase: "https://api.mistral.ai/v1",
    fetchModelsStrategy: "openai-compatible",
    defaultPricing: { inputPer1M: 0.5, outputPer1M: 1.5 },
  },
  {
    name: "xAI (Grok)",
    needsCcr: false,
    directApi: true,
    directApiBase: "https://api.x.ai/v1",
    fetchModelsStrategy: "openai-compatible",
    defaultPricing: { inputPer1M: 2, outputPer1M: 8 },
  },
  {
    name: "Cohere",
    needsCcr: false,
    directApi: true,
    directApiBase: "https://api.cohere.com/v1",
    fetchModelsStrategy: "openai-compatible",
    defaultPricing: { inputPer1M: 0.5, outputPer1M: 1.5 },
  },
  {
    ...
    needsCcr: false,
    directApi: true,
    directApiBase: "https://api.together.xyz/v1",
    fetchModelsStrategy: "openai-compatible",
    defaultPricing: { inputPer1M: 0.5, outputPer1M: 1.5 },
  },
  {
    ...
    fetchModelsStrategy: "openai-compatible",
    defaultPricing: { inputPer1M: 1, outputPer1M: 5 },
  },
  {
    name: "Fireworks AI",
    needsCcr: false,
    directApi: true,
    directApiBase: "https://api.fireworks.ai/v1",
    fetchModelsStrategy: "openai-compatible",
    defaultPricing: { inputPer1M: 0.2, outputPer1M: 0.6 },
  },
...
function buildTokenPricingFromProviders(providers) {
  ...
  const list = Array.isArray(providers)
    ? providers
    : Object.values(loadStore().providers || {});
  for (const p of list) {
...
function inferDirectApiFromModel(modelName) {
  ...
  for (const p of KNOWN_CLOUD_PROVIDERS) {
    ...
    if (
      m.includes(pn) ||
      pn.split(/[\s(]/).some((part) => part.length > 2 && m.includes(part))
    ) {
  ...
  if (
    /gemini/i.test(m) ||
    (/^\d[\w.-]*/.test(m) &&
      /(flash|live|exp|pro|thinking|vision|ultra)/i.test(m))
  ) {
    const gemini = KNOWN_CLOUD_PROVIDERS.find(
      (p) => p.name === "Google Gemini",
    );
    if (gemini && gemini.directApi && gemini.directApiBase) {
      return {
        direct: true,
        apiBase: gemini.directApiBase,
        providerName: gemini.name,
      };
    }
  }
...
async function fetchProviderModels({
  ...
  if (!providerName)
    return { ok: false, error: "供应商名称不能为空", models: [] };
  ...
  if (!resolvedKey)
    return {
      ok: false,
      error: "API Key 不能为空（已保存的供应商中未找到该名称）",
      models: [],
    };
  ...
  if (!strategy) {
    if (
      /generativelanguage\.googleapis/i.test(base) ||
      /gemini/i.test(providerName)
    ) {
      strategy = "gemini";
    } else {
  ...
  try {
    ...
    if (strategy === "gemini") {
      ...
      models = rawModels
        .filter((m) => {
          ...
          return (
            ...
            methods.some((s) => /generateContent/i.test(s))
          );
        })
        .map((m) =>
          String(m.name || "")
      ...
      if (modelsPath) {
        ...
        let originBase = base || "";
        if (!originBase)
          originBase = known?.directApiBase || "https://api.openai.com";
        try {
      ...
      if (status !== 200)
        return { ok: false, error: `API 返回 ${status}`, models: [] };
      const rawModels = Array.isArray(data?.data) ? data.data : [];
      models = rawModels.map((m) => String(m.id || "").trim()).filter(Boolean);
    }
...
function resolveHostnameSimple(hostname) {
  ...
  const tryResolve = (servers) =>
    new Promise((resolve) => {
      ...
      resolver.resolve4(hostname, (err, addresses) => {
        ...
        resolve(
          ip === "127.0.0.1" || ip === "0.0.0.0" || ip.startsWith("192.168.")
            ? ""
            : ip,
        );
      });
  ...
  return (async () => {
    ...
    try {
      ...
      const ip =
        Array.isArray(addresses) && addresses.length ? addresses[0] : "";
      return ip &&
        ip !== "127.0.0.1" &&
        ip !== "0.0.0.0" &&
...
function httpGetJson(url, timeoutMs, headers = {}) {
  return new Promise((resolve, reject) => {
    ...
    // 先解析 DNS，避免被本地代理劫持
    resolveHostnameSimple(hostname)
      .then((realIp) => {
        const opts = {
...
function buildProviderEnv(input) {
  ...
  const sonnet = String(
    input.sonnetModel || input.model || input.defaultModel || "",
  ).trim();
...
function normalizeStoreModelIds() {
  ...
  for (const rec of Object.values(store.providers || {})) {
    const next = [
      ...new Set(
        (rec.models || []).map((m) => normalizeCloudModelId(m)).filter(Boolean),
      ),
    ];
...
function findProviderForModel(modelId, store = loadStore()) {
  ...
  const modelInList = (rec, id) => {
    ...
    if (list.includes(model) || list.includes(String(modelId || "").trim()))
      return true;
...
function upsertProvider(input) {
  ...
  const knownDefault =
    KNOWN_CLOUD_PROVIDERS.find((p) => p.name === name)?.defaultPricing || {};
  const inputPrice = Number(
    input.inputPrice ?? input.inputPer1M ?? knownDefault.inputPer1M ?? 0,
  );
  const outputPrice = Number(
    input.outputPrice ?? input.outputPer1M ?? knownDefault.outputPer1M ?? 0,
  );
  store.providers[id] = {
    id,
    name,
    ...
    outputPrice,
    currency:
      String(input.currency || existing?.currency || "")
        .trim()
        .toUpperCase() || "USD",
    websiteUrl: String(
...
async function collectCloudModelPool({
  aliases = [],
  settings = {},
  fetchRemote = true,
} = {}) {
...
async function syncProvidersToWorkbench({
  loadChatSettings,
  saveChatSettings,
  loadChatSessions,
  saveChatSessions,
  ...
  if (!rec)
    throw new Error("请先在「设置 → 模型与连接」添加并启用云模型供应商");
  ...
  const nextSettings = {
    ...
    cloudModelCatalog: await filterCloudModelList(pool, cur),
  };
